-- ============================================================================
-- Migración: Completar entity_label de log_change() para las tablas que
-- caían en el fallback genérico "id=X" (fix-097-m)
-- ============================================================================
-- Problema: la función log_change() (redefinida en 20260614201000) solo
-- resolvía un nombre legible para enrollments, payments,
-- standalone_course_enrollments, special_service_sales, class_b_sessions,
-- users, students y professional_pre_registrations. El resto de tablas con
-- trigger activo (student_documents, certificates, vehicles,
-- vehicle_documents, maintenance_records, class_b_theory_sessions,
-- class_book, professional_theory_sessions, professional_practice_sessions,
-- professional_module_grades, promotion_courses) caían al ELSE genérico
-- ('id=' || id), sin valor para quien audita (ej. "Creado: id=97").
--
-- Solución: agregar una rama ELSIF por tabla que arma un v_entity_label
-- legible, manteniendo intactas todas las ramas existentes y el resto de
-- la función (resolución de usuario, diff de UPDATE, INSERT/DELETE).
-- ============================================================================
...
BEGIN
  ...
  -- ── Tablas agregadas en fix-097-m (antes caían al ELSE genérico) ─────────
...
COMMENT ON FUNCTION public.log_change() IS
  'Trigger de auditoría (fix-097-m): agrega entity_label legible para student_documents, certificates, vehicles, vehicle_documents, maintenance_records, class_b_theory_sessions, promotion_courses, class_book, professional_theory_sessions, professional_practice_sessions y professional_module_grades — antes caían al fallback genérico id=X.';
