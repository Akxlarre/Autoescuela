...
| `EnrollmentSummary`, `AlumnoDetalleUI`, `ProgresoAsistenciaProf`, `ElegibilidadProfUI`, `PagoUI`, `InasistenciaUI`, `InasistenciaClaseBUI`, `ClasePracticaUI`, `ClasePendienteReagendarUI`, `ProgresoUI` | `ui` | `src/app/core/models/ui/alumno-detalle.model.ts` — `ClasePracticaUI.ausente` (RF-053, `class_b_sessions.status='no_show'`) y `ClasePracticaUI.cancelada` (`status='cancelled'` — bucket visual propio en la grilla; antes caía en el bucket "azul/agendada" porque `cancelled` conserva `scheduled_at`); `ClasePracticaUI.justificada`/`justificacion` (derivados de `class_b_practice_attendance.status='excused'`/`.justification` para esa sesión, vía mapa `session_id → attendance` construido en `fetchClassBProgress()` — la clase se mantiene `ausente=true` (rojo) aun justificada, solo cambia el subtítulo); `InasistenciaClaseBUI` (derivado de `class_b_practice_attendance`; distinto de `InasistenciaUI`/`absence_evidence`, que sigue siendo exclusivo de Profesional; campo `reagendada: boolean` — soft archive RF-053: `true` cuando `class_b_sessions.status !== 'no_show'` para la sesión asociada, es decir, la sesión ya fue reciclada por el flujo de reagendar pero el registro de inasistencia se conserva para auditoría — badge "Reagendada" en `AdminAlumnoDetalleComponent`); `ClasePendienteReagendarUI` (RF-053 — `{sessionId, claseNumero, origen: 'no_show'|'cancelled', fechaOriginal, justificada}`, alimenta el checklist de `AdminReagendarClasesDrawerComponent`; `justificada` reusa el mismo mapa `attendanceBySessionId` de `ClasePracticaUI.justificada` — badge se rotula "Inasistencia — Justificada" en vez de solo "Inasistencia", sigue en rojo) |
...
| `DmsTab`, `TemplateCategory`, `TemplateCategoryFilter`, `StudentWithDocsRow`, `DmsStudentDocRow`, `SchoolDocRow`, `TemplateCard`, `DmsKpis`, `UploadStudentDocPayload`, `UploadSchoolDocPayload`, `UploadTemplatePayload`, `DmsViewerDocument` | `ui` | `src/app/core/models/ui/dms.model.ts` — `DmsStudentDocRow.source` ampliado a `'student_document' \| 'digital_contract' \| 'enrollment_license'` (fix desconexión DMS/Carnet): el nuevo valor identifica filas sintéticas derivadas de `enrollments.license_initial_url`/`license_full_url` vía la vista `v_dms_student_documents` (extensión SQL pendiente de aplicar por el humano, ver `indices/DATABASE.md`) — sin fila propia en ninguna tabla, por lo que no son eliminables desde el DMS. |
