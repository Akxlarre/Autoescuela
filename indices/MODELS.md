...
| `DocumentTemplate`, `DocumentType` | `document-template.model.ts` | **Spec 0016-m.** Reescrita por completo — antes mapeaba archivos descargables (`file_url`/`format`/`download_count`, en desuso real), ahora mapea el contenido editable por cláusula/sección de contratos y certificados generados vía Edge Function (`branch_id`, `document_type`, `content: Record<string,string>`). `DocumentType` = `'contract_b'\|'contract_professional'\|'certificate_b'\|'certificate_professional'`. |
...
| `DocumentTemplateSection`, `DocumentTemplateForm` | `document-content-template.model.ts` | **Spec 0016-m.** Forma editable del editor de plantillas — no basta el DTO porque el JSON de `content` no trae labels legibles ni los tokens `{{token}}` disponibles por cláusula (metadata estática, no viene de BD). Constante `DOCUMENT_TEMPLATE_SECTIONS: Record<DocumentType, ...>` define label/maxLength/tokens por sección de cada uno de los 4 tipos de documento (ver tabla completa en `specs/specs/0016-m-editor-plantillas-documentos/plan.md` §4). `DOCUMENT_TYPE_LABELS` para el selector de tipo. |
...
| `DiscountType`, `DiscountApplicableTo`, `DiscountStatus`, `DiscountUi`, `DiscountFormValue` | `ui` | `src/app/core/models/ui/discount.model.ts` |
| `DmsTab`, `StudentWithDocsRow`, `StudentEnrollmentOption`, `DmsStudentDocRow`, `InstructorWithDocsRow`, `DmsInstructorDocRow`, `SchoolDocRow`, `DmsKpis`, `UploadStudentDocPayload`, `UploadSchoolDocPayload`, `UploadInstructorDocPayload`, `DmsViewerDocument` | `ui` | `src/app/core/models/ui/dms.model.ts` |
| `DocumentTemplateSection`, `DocumentTemplateForm` | `ui` | `src/app/core/models/ui/document-content-template.model.ts` |
