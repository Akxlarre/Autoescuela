...
| `InstructorEvaluacionComponent` (Drawer, sin ruta desde fix-236-m) | `InstructorEvaluacionComponent` | N/A — ya no es página enrutable | **fix-236-m:** la ruta `/instructor/alumnos/:id/evaluacion/:sessionId` se eliminó; la evaluación se abre en el Drawer lateral (`LayoutDrawerFacadeService`) desde la ficha del alumno y el horario. Fuera del rollout app-like por construcción — un Drawer no es una página de contenido enrutable. |
