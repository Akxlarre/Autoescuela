...
|---|---|---|---|---|---|
| C1 | ✅ **Clase completada** (práctica B) — implementado Spec 0026, extendido fix-091-m (ASG-b-044), corregido fix-148-m | `InstructorClasesFacade.finishClass()` / `AsistenciaClaseBFacade.finalizarClase()` (mismo `class_b_sessions.status='completed'`, usado también por secretaría/admin) → trigger `notify_class_b_completed()` | Alumno ("Clase N/6 o N/12 completada" — total dinámico según `courses.is_reinforcement`, fix-148-m) **+ todas las secretarias de la sede, solo si el actor es instructor** (`auth_user_role()='instructor'`, fix-148-m evita autonotificación cuando cierra secretaría/admin) — mensaje "Clase N/6 o N/12 de matrícula #{number} (PrimerNombre ApellidoPaterno) completada por el instructor" | `class_b` | Trigger `AFTER UPDATE OF status ON class_b_sessions` (`20260710000000` → `20260801100000` → `20260810120000`) |
