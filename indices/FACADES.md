...
| `AdminAlumnoDetalleFacade` | `core/facades/admin-alumno-detalle.facade.ts` | Ficha del Alumno. `loadDetalle(studentId)` — 3 queries en paralelo con `Promise.all`: (1) `students!inner(users, enrollments(courses))` → mapea a `AlumnoDetalleUI`; (2) `class_b_practice_attendance` → cuenta status=`presente` para `progresoPractico`; (3) `class_b_theory_attendance` → cuenta status=`presente` para `progresoTeorico`. Filtra status=`ausente` de ambas tablas y los combina en `inasistencias` (tipo Práctica/Teórica, estado Justificada/Injustificada por campo `justification`). Constantes `PRACTICAS_REQUERIDAS_B=12`, `TEORICAS_REQUERIDAS_B=8` (TODO: derivar de courses). Computed: `porcentajePracticas`, `porcentajeTeoricas`. Usado por `AdminAlumnoDetalleComponent`. |
