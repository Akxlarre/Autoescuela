...
| `EnrollmentFacade` | Orquestador del wizard de matrícula (6 pasos). Draft progresivo con `expires_at` +24h. **Step 1:** `findUserByRut()`, `savePersonalData()` → upsert user+student, crear enrollment draft. **Step 2:** `loadInstructors()`, `loadScheduleGrid()` (vista `v_class_b_schedule_availability`), `loadPromotions()`, `saveAssignment()` → reservar sesiones o asignar promoción. **Step 5:** `generateContract()` (Edge Function), `uploadSignedContract()` → storage + `digital_contracts` + `contract_accepted=true` + `goToStep(6)`. `markContractSigned({ signerName, signatureHash, signedAt })` → firma digital sin archivo físico; upsert `digital_contracts` + `contract_accepted=true` + `goToStep(6)`. **Step 6:** `confirmEnrollment()` → genera número YYYY-NNNN, status → active. **Wizard:** signals privados (readonly), computed `canAdvance`, `studentSummary`, `sidebarSummary`. `courseOptions` computed public (`CourseOption[]`). `reset()`, `goToStep()`, `goBack()`. | `core/facades/enrollment.facade.ts` | SupabaseService | ✅ Estable |
