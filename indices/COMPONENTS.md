...
| `/app/admin/alumnos/:id` | **Ficha del Alumno** — Inyecta `AdminAlumnoDetalleFacade`. Lee `:id` de `ActivatedRoute` → `loadDetalle(id)` en `ngOnInit`. **3 estados:** skeleton (`app-skeleton-block`), error card, vista principal con `@else if (facade.alumno(); as alumno)`. **Datos reales (Facade):** Info Personal, barras de progreso Prácticas/Teóricas desde `progresoPractico`/`progresoTeorico`, Inasistencias (Justificada/Injustificada). **Mock pendiente BD:** Ficha Técnica 12 filas + firmas, Historial de Pagos 4 registros. Botón "Volver al Listado". Abre `AdminInasistenciaDrawerComponent` vía `drawerOpen` signal; al guardar llama `onInasistenciaGuardada()` que recarga el detalle. | `AdminAlumnoDetalleFacade` | `features/admin/alumno-detalle/admin-alumno-detalle.component.ts` | ✅ Estable |
