...
| `app-assignment-step` | Organismo / Matricula | `data` (input req): `EnrollmentAssignmentData` — incluye `instructors[]`, `scheduleGrid`, `scheduleLoading`, `promotionGroups: PromotionGroup[]`. **Clase B:** selector instructor + semana + slots. Navegación de semanas: `currentWeekIndex`, `weeks` (agrupa `daysFromGrid()` por lunes vía `getMondayKey()`), `prevWeek/nextWeek`. Counter `X/N` = `requiredCount` (practical_hours × 60 / 45). **Profesional:** itera `promotionGroups[]` → `@for (group)` → `@for (option)` — vacío muestra estado empty; opción `status=finished` deshabilitada. **Singular:** panel informativo. | `shared/components/matricula-steps/assignment/assignment.component.ts` | ✅ Estable |
