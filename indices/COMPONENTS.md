...
| `/app/admin/pagos` | **Gestión de Pagos** — Inyecta `PagosFacade`. `ngOnInit` llama `initialize()` (patrón SWR). Cabecera con título + botón "+ Registrar Pago". Grid 4 KPIs (`app-kpi-card-variant`): Ingresos Hoy, Ingresos Mes, Pagos Pendientes (warning), Boletas Emitidas. Valores monetarios en formato compacto K/M con subValue CLP exacto (`formatCLP`). Tabla "Alumnos con saldo pendiente": 6 columnas (ALUMNO, RUT, TOTAL A PAGAR, PAGADO en verde, SALDO en warning, ACCIONES). Skeleton de 4 filas. Estado vacío con check-circle. | `PagosFacade` | `features/admin/pagos/admin-pagos.component.ts` | ✅ UI Ready |
