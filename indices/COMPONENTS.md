...
| `/app/admin/pagos` | **Gestión de Pagos** — Inyecta `PagosFacade`. `ngOnInit` llama `initialize()` (SWR). **Secciones:** (1) Cabecera + botón "+ Registrar Pago". (2) Grid 4 KPIs (`app-kpi-card-variant`): Ingresos Hoy/Mes (formato K/M con subValue CLP exacto), Pagos Pendientes (warning), Boletas Emitidas. (3) Tabla "Alumnos con saldo pendiente": 6 cols, skeleton 4 filas, empty state. (4) Barra de búsqueda + selects "estado/método" con filtrado client-side via `computed()`. (5) Layout `grid-cols-3`: izquierda "Pagos Recientes" (col-span-2) con tabla 7 cols, paginación 5/página, badges estado coloreados con `color-mix`; derecha (col-span-1) "Métodos de Pago" con barras `[style.width.%]` + colores CSS vars, "Accesos Rápidos" (3 botones), "Recordatorio". | `PagosFacade` | `features/admin/pagos/admin-pagos.component.ts` | ✅ UI Ready |
