...
| `/app/admin/alumnos/:id` | **Ficha del Alumno** — Inyecta `AdminAlumnoDetalleFacade`. Lee `:id` de `ActivatedRoute` → `loadDetalle(id)` en `ngOnInit`. **3 estados:** skeleton, error card, vista principal. **Datos reales:** Info Personal, barras de progreso, Inasistencias (`facade.inasistencias()`), Ficha Técnica (`facade.clasesPracticas()` — 12 filas, firmas `alumnoFirmo`/`instructorFirmo`), Historial de Pagos (`facade.historialPagos()` — totales `alumno.totalPagado`/`alumno.saldoPendiente` desde `enrollments`). Botón "Volver al Listado". Abre `AdminInasistenciaDrawerComponent` via `drawerOpen` signal; `(saved)` → `onInasistenciaGuardada()` recarga. Helper `statusLabel()` para badges de inasistencias. | `AdminAlumnoDetalleFacade` | `features/admin/alumno-detalle/admin-alumno-detalle.component.ts` | ✅ Estable |
