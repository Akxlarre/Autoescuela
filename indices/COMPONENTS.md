...
| Componente | Tipo/Categoría | Props principales | Ubicación | Estado |
|------------|----------------|-------------------|-----------|--------|
| `app-agenda-semanal` | Organismo / Agenda (Dumb) | `weekData` (`AgendaWeekData\|null`), `filteredDays` (`AgendaDayColumn[]`), `timeRows` (`string[]`), `isLoading` (boolean), `isCurrentWeek` (boolean), `instructors` (`AgendaInstructorFilter[]`), `selectedInstructorId` (`number\|null`) — outputs: `weekNext`, `weekPrev`, `weekToday`, `instructorFilterChange` (number\|null), `slotClick` (AgendaSlot). Hero con semana, 4 KPI cards, toolbar nav + filtro instructor, CSS grid de calendario Lun-Vie, leyenda. GSAP en bento KPIs. | `shared/components/agenda-semanal/agenda-semanal.component.ts` | ✅ Estable |
| `app-agenda-slot` | Átomo / Agenda (Dumb) | `slot` (`AgendaSlot`, req) — output: `slotClicked` (AgendaSlot). Renderiza un bloque individual del calendario: available (dashed, clickeable), scheduled/in_progress (brand), completed (success), cancelled/no_show (muted). | `shared/components/agenda-semanal/agenda-slot.component.ts` | ✅ Estable |
