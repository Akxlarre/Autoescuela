...
| `/app/admin/alumnos/:id` | **Ficha del Alumno (UI completa, mock)** — Breadcrumbs, header + 3 botones (Generar Carnet, Generar Certificado deshabilitado, Editar), grid 3 columnas (Info Personal, Clases Prácticas con barra 8/12 67%, Clases Teóricas 6/8 75%), sección Inasistencias (warning-bg, badge Justificada/Injustificada), Ficha Técnica tabla 12 filas (8 completadas con firmas verde/azul + 4 pendientes), Historial de Pagos (4 registros, resumen Total/Saldo, badge Pagado/Pendiente, footer con enlace módulo pagos), botón "Volver al Listado". **Sin Facade aún — todos los datos son mock.** | — | `features/admin/alumno-detalle/admin-alumno-detalle.component.ts` | ✅ UI Ready |
