...
| `apply_class_b_absence_penalty(p_enrollment_id INT)` | `20260705000000_class_b_absence_auto_penalty.sql` | Llamada por `mark_end_of_day_class_b_absences()` y por `AsistenciaClaseBFacade.markAttendance()` vía `supabase.rpc()` | RF-053: 2 inasistencias no justificadas = pérdida de agenda. Cuenta `class_b_practice_attendance` con `status IN ('absent','no_show')` de la matrícula (excluye `'excused'` = justificadas); si el conteo es `>= 2`, cancela (`status='cancelled', cancelled_at=NOW()`) todas las `class_b_sessions` `'scheduled'` con `scheduled_at > NOW()` de esa matrícula, liberando instructor/vehículo. Retorna `INT` con la cantidad cancelada (`0` si no aplica penalización). `SECURITY DEFINER`. |
