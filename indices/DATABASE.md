...
|-------|---------|-------------|--------------------------|
| `v_student_progress_b` | M4 - Acad. B | Progreso prácticas (0-12) `completed_practices`/`pct_practices`/`last_practice_session` por matrícula Clase B. **`20260630000000` (Spec 0001): eliminada `pct_theory_attendance`** — la asistencia teórica ya no existe. | Admin, Sec, Inst (propias), Stu (propia) |
...
| `mark_end_of_day_class_b_absences()` | `20260705000000_class_b_absence_auto_penalty.sql` | pg_cron: `0 1 * * *` (diario 01:00 UTC ≈ 21:00 CLT invierno, fin de jornada) | RF-053. Recorre `class_b_sessions` en `status='scheduled'` cuya fecha (America/Santiago) sea hoy o anterior y las marca `status='no_show'`, insertando `class_b_practice_attendance(status='absent')` por alumno (`ON CONFLICT DO NOTHING`, idempotente). Compara por fecha calendario en America/Santiago (no en UTC del servidor) para evaluar correctamente "el día que recién termina" pese a que el cron corre pasada la medianoche UTC. Por cada matrícula afectada invoca `apply_class_b_absence_penalty()`. `SECURITY DEFINER`. |
| `apply_class_b_absence_penalty(p_enrollment_id INT)` | `20260705000000_class_b_absence_auto_penalty.sql` → **corregida en `20260706000000_fix_class_b_absence_penalty_consecutive.sql`** | Llamada por `mark_end_of_day_class_b_absences()` y por `AsistenciaClaseBFacade.markAttendance()` vía `supabase.rpc()` | RF-053: 2 inasistencias no justificadas **consecutivas** = pérdida de agenda. Busca dos `class_b_sessions` de `class_number` adyacente (N, N+1) de la matrícula donde AMBAS tengan `class_b_practice_attendance.status IN ('absent','no_show')` (excluye `'excused'` = justificadas); si existen, cancela (`status='cancelled', cancelled_at=NOW()`) todas las `class_b_sessions` `'scheduled'` con `scheduled_at > NOW()` de esa matrícula, liberando instructor/vehículo. Una clase `'present'` o `'excused'` entre dos inasistencias rompe la secuencia y NO penaliza (antes de la corrección, contaba el total acumulado de la matrícula sin importar si estaban separadas por asistencias normales). Retorna `INT` con la cantidad cancelada (`0` si no aplica penalización). `SECURITY DEFINER`. |
...
> Asistencia a clases prácticas individuales Clase B. RF-053: 2 inasistencias no justificadas = pérdida de agenda futura. Estados: `present` | `absent` | `excused` (justificada, vía `AsistenciaClaseBFacade.justifyAbsence()`) | `no_show`. Ver `apply_class_b_absence_penalty()` y `mark_end_of_day_class_b_absences()` (`20260705000000`).
