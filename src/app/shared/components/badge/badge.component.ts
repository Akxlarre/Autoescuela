...
@Component({
  ...
  template: `
    <span class="inline-flex items-center justify-center px-2.5 py-1 text-xs font-semibold rounded-full" [class]="getClasses()">
...
export class BadgeComponent {
  ...
  getClasses() {
    switch (this.variant) {
      ...
      case 'error': return 'bg-[var(--state-error-bg)] text-[var(--state-error)] dark:bg-[var(--state-error-bg)]/30';
      case 'info': return 'bg-brand-muted text-brand-primary dark:bg-brand-muted/30';
      case 'neutral': return 'bg-[var(--bg-subtle)] text-[var(--text-secondary)]';
      default: 
