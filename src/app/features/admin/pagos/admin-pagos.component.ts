      ...
      >
        ...
        <div class="flex-1 min-h-0 overflow-y-auto">
          @if (facade.isLoading()) {
            ...
            <div
              class="micro-label hidden lg:grid px-6 py-2 gap-4 border-b bg-surface border-border-muted"
              [class.deudores-grid-cols-sede]="mostrarColumnaSede()"
              [class.deudores-grid-cols]="!mostrarColumnaSede()"
            >
              <span>Alumno</span>
              <span class="dc-rut">RUT</span>
              @if (mostrarColumnaSede()) {
                <span class="dc-sede">Sede</span>
              }
            ...
            <div class="rows-divider">
              @for (alumno of deudoresVisibles(); track alumno.enrollmentId) {
                <div
                  class="deudores-row p-4 lg:px-6 lg:py-4 flex flex-col lg:grid lg:gap-4 lg:items-center transition-colors"
                  [class.deudores-grid-cols-sede]="mostrarColumnaSede()"
                  [class.deudores-grid-cols]="!mostrarColumnaSede()"
                >
                  ...
                  <!-- Sede Desktop only -->
                  @if (mostrarColumnaSede()) {
                    <span class="hidden lg:block text-sm text-text-secondary truncate dc-sede">
                      {{ alumno.sedeNombre }}
                    </span>
                  }

                  <!-- Fecha Matrícula Desktop only -->
...
export class AdminPagosComponent implements AfterViewInit {
  ...
  // Sin filtro de sede: si el admin quiere ver una sede en particular, ya tiene el
  // selector de sede del topbar para eso. La columna sí se mantiene — cuando el
  // topbar está en "Todas las sedes" ayuda a distinguir a qué sede pertenece cada fila.
  protected readonly mostrarColumnaSede = computed(
  ...
  protected readonly totalPaginasDeudores = computed(() =>
    Math.max(
      1,
      Math.ceil(this.deudoresFiltrados().length / AdminPagosComponent.DEUDORES_PAGE_SIZE),
    ),
  );
