  ...
  template: `
    <div class="px-6 py-6 pb-20 max-w-7xl mx-auto space-y-6">
      ...
      <section
        ...
        appBentoGridLayout
        #bentoRef
        aria-label="Métricas de cursos singulares"
      >
      ...
      <!-- ── Tabla de cursos ────────────────────────────────────────────────── -->
      <section class="card" aria-label="Listado de cursos singulares">
...
export class AdminContabilidadCursosComponent implements OnInit, AfterViewInit {
  ...
  // ── GSAP ref ───────────────────────────────────────────────────────────────
  private readonly bentoRef = viewChild<ElementRef<HTMLElement>>('bentoRef');
  ...
  ngAfterViewInit(): void {
    const bento = this.bentoRef()?.nativeElement;
    if (bento) this.gsap.animateBentoGrid(bento);
  }
