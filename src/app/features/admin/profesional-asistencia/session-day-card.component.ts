...
@Component({
  ...
  template: `
    <div
      ...
      [style.border]="
        day().isToday ? '1px solid var(--ds-brand)' : '1px solid var(--border-subtle)'
      "
      ...
      <div class="flex flex-col gap-2.5 flex-1">
        ...
        @if (day().theory; as session) {
          <button
            ...
            [style.background]="
              session.status === 'completed' ? 'var(--bg-surface)' : 'var(--bg-base)'
            "
            [style.border]="
          ...
          >
            ...
            <div
              ...
              [style.background]="getStatusColor(session)"
            ></div>
...
export class SessionDayCardComponent {
  ...
  getStatusBgColor(session: SesionProfesional): string {
    ...
    switch (session.status) {
      ...
      case 'in_progress':
        return 'color-mix(in srgb, var(--ds-brand) 10%, transparent)';
      case 'cancelled':
