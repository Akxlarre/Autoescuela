  ...
  template: `
    <div class="flex-1 flex flex-col min-h-0 bg-surface">
        ...
        @if (flotaFacade.isLoadingAgenda()) {
          ...
          <div class="space-y-3">
            @for (slot of flotaFacade.vehicleAgenda(); track slot.hour) {
              <div class="group p-4 rounded-xl border bg-base hover:border-ds-brand hover:shadow-sm transition-all cursor-default"
                   [ngClass]="{'opacity-50': slot.type === 'empty'}">
                <div class="flex items-start justify-between">
                  <div class="flex items-center gap-3">
                    <div class="w-12 h-10 rounded-lg bg-ds-brand-muted flex items-center justify-center text-ds-brand">
                    ...
                    <div>
                      @if (slot.type === 'class') {
                        ...
                        <p class="text-[11px] text-text-muted uppercase tracking-wider">Mantenimiento</p>
                      } @else {
                  ...
                  @if (slot.type !== 'empty') {
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center bg-subtle text-text-muted">
      ...
      <div class="shrink-0 p-6 border-t bg-surface flex items-center justify-end" style="border-color: var(--border-subtle);">
        <button
          type="button"
          class="h-11 px-8 rounded-xl bg-ds-brand text-white border-none cursor-pointer font-semibold shadow-sm hover:opacity-90 transition-all active:scale-95 flex items-center gap-2"
          (click)="onClose()"
        >
          Cerrar Agenda
        </button>
