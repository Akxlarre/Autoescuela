      ...
      <ng-template #content>
        <div class="flex-1 overflow-y-auto p-5">
          ...
          @if (facade.isLoading()) {
            ...
            <div
              class="flex items-start gap-2 rounded-xl p-3 bg-warning/10"
          ...
          <div class="flex flex-col gap-1.5 mt-3">
            @if (facade.isLoading()) {
              ...
              <p class="text-sm py-4 text-center text-text-muted">
                No hay alumnos activos de Clase B.
              </p>
            } @else {
              @for (alumno of facade.alumnos(); track alumno.studentId) {
                <label
                  class="flex items-center gap-3 py-2.5 px-3 rounded-xl cursor-pointer transition-colors border border-border-subtle"
                  [style.background]="
                ...
                >
                  ...
                  <div class="flex flex-col gap-0 min-w-0">
                    ...
                    <span class="text-xs truncate text-text-muted">{{ alumno.email }}</span>
                  </div>
                </label>
              }
            }
          ...
          @if (facade.savedOk()) {
            <div
              class="flex items-center gap-2 px-4 py-3 rounded-xl mt-4 bg-success/12"
          ...
          @if (facade.error()) {
            <div
              class="flex items-center gap-2 px-4 py-3 rounded-xl mt-4 bg-error/12"
              
            >
              <app-icon name="circle-alert" [size]="16" class="text-error" />
              <span class="text-sm text-error">{{ facade.error() }}</span>
            </div>
          }
        </div>
