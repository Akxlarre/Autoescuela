...
@Component({
  ...
  template: `
    ...
    >
      ...
      <!-- ── Stepper + error (1 sola fila auto) ─────────────────────────────────
           Wrapper SIEMPRE presente: el error es condicional y se sumaba como fila
           extra, corriendo el contenido del paso fuera de la fila fill. Mismo
           mecanismo que en clases/pagos (fix-127-b). -->
      <div class="bento-banner flex flex-col gap-3">
        <div class="card px-6 py-4" appCardHover>
      ...
      <!-- ── Contenido del paso — celda protagonista (fila fill, scroll interno) ──
           Step 1 y Step 3 son mutuamente excluyentes, así que comparten la única
           celda .bento-fill. Medido con saldo pendiente real: el step 1 desbordaba
           desde 1440×768 y el step 3 a 1440×700, así que la página NO califica por
           el criterio #1 y sí necesita el patrón. -->
        ...
        <div class="flex-1 min-h-0 overflow-y-auto flex flex-col">
          ...
          @if (facade.step() === 1) {
            @if (facade.isLoading()) {
              ...
              <!-- Centrado en el alto disponible: dentro del .bento-fill la celda mide
                   el resto del viewport y esta card quedaría arriba con un hueco debajo. -->
              <div
                class="card p-8 flex-1 flex flex-col items-center justify-center gap-4 text-center"
                appCardHover
              >
                <div
                  class="w-14 h-14 rounded-full flex items-center justify-center bg-success-subtle"
                >
                ...
                <div class="flex flex-col gap-4">
                  <div class="card p-5 flex flex-col gap-3" appCardHover>
                    ...
                    <p class="text-xs text-text-muted">
                      Tu horario quedó fijado durante el proceso de matrícula. Solo debes completar
                      el pago.
                    </p>
                  </div>
                  <button
            ...
            <div class="flex flex-col gap-4">
              <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                ...
                <div class="flex flex-col gap-4">
                  ...
                  <div
                    class="flex items-start gap-3 p-3 rounded-lg text-xs text-text-muted bg-surface"
                  >
                    <app-icon name="credit-card" [size]="14" class="shrink-0 mt-0.5" />
                    <span
                      >Serás redirigido al portal seguro de Transbank (Webpay Plus) para completar
                      el pago con tarjeta de débito o crédito.</span
                    >
                  </div>
