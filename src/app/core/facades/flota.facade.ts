...
export class FlotaFacade {
  ...
  setupRealtime(): void {
    ...
    this._realtimeChannel = this.supabase.client
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'vehicles' }, () => void this.refreshSilently())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'vehicle_documents' }, () => void this.refreshSilently())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'vehicle_assignments' }, () => void this.refreshSilently())
      .subscribe();
  }
  ...
  async init(): Promise<void> { return this.initialize(); } // Alias legacy
  ...
  setTypeFilter(type: VehicleType | null): void { this._typeFilter.set(type); }
  setStatusFilter(status: VehicleStatus | null): void { this._statusFilter.set(status); }
  selectVehicle(id: number | null): void { this._selectedVehicleId.set(id); }
  ...
  async upsertVehicleDocument(payload: any): Promise<void> {
    const { error } = await this.supabase.client.from('vehicle_documents').upsert({ ...payload, status: 'valid' }, { onConflict: 'vehicle_id,type' });
    if (error) throw error;
    void this.refreshSilently();
  }
  ...
  async loadVehicleAgenda(vehicleId: number, date: Date): Promise<void> {
    ...
    try {
      ...
      const [sessionsResult, maintenancesResult] = await Promise.all([
        this.supabase.client.from('class_b_sessions').select('id, scheduled_at, status, class_number, enrollments!inner(students!inner(users!inner(first_names, paternal_last_name)))').eq('vehicle_id', vehicleId).gte('scheduled_at', `${dateStr}T00:00:00`).lte('scheduled_at', `${dateStr}T23:59:59`).order('scheduled_at', { ascending: true }),
        this.supabase.client.from('maintenance_records').select('id, scheduled_date, type, description').eq('vehicle_id', vehicleId).eq('scheduled_date', dateStr)
      ]);

      const slots: VehicleAgendaSlot[] = [];
      const sessionMap = new Map((sessionsResult.data ?? []).map((s: any) => [new Date(s.scheduled_at).getHours(), s]));
      ...
      for (let h = 8; h < 18; h++) {
        ...
        if (ses) {
          ...
          slots.push({ hour, endHour: `${String(h + 1).padStart(2, '0')}:00`, type: 'class', studentName: user ? `${user.first_names} ${user.paternal_last_name}`.trim() : 'Alumno', classNumber: ses.class_number });
        } else {
  ...
  private resolveType(brand: string, model: string): VehicleType {
    ...
    return ['atego', 'actros', 'accelo', 'camion', 'bus', 'volvo', 'iveco'].some(k => label.includes(k)) ? 'professional' : 'class_b';
  }

  private resolveStatus(raw: string | null): VehicleStatus {
    const map: Record<string, VehicleStatus> = { available: 'available', disponible: 'available', in_class: 'in_class', 'en clase': 'in_class', maintenance: 'maintenance', mantenimiento: 'maintenance', out_of_service: 'out_of_service' };
