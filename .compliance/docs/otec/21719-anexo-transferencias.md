# Anexo de Transferencia Internacional de Datos — Jorge Enrique Pérez Godoy Capacitación y Servicios EIRL
...
- **Exportador de datos:** Jorge Enrique Pérez Godoy Capacitación y Servicios EIRL, RUT 76.007.217-6,
  domicilio en Maipón 418, Chillán, Región de Ñuble (Chile).
...
|---|---|---|---|---|---|---|---|
| 1 | **Supabase Inc.** | Estados Unidos `[CONFIRMAR la región concreta del proyecto en el panel de Supabase — puede ser EE.UU. o Sudamérica; si el proyecto está en São Paulo, el destino es Brasil]` | La totalidad de la base de datos: identificación de alumnos, relatores y trabajadores, expedientes con **certificado médico y cédula**, académicos, datos laborales de alumnos enviados por empresas clientes, pagos, registros de auditoría con IP | Infraestructura de base de datos, almacenamiento de archivos y autenticación | Encargado | Cláusulas modelo | ❌ **Pendiente de incorporar** |
...
> las empresas clientes** que envían trabajadores a capacitarse. Respecto de esos datos la OTEC actúa
> como **encargado**, y **subcontratar infraestructura extranjera requiere la autorización de la empresa
> cliente**. Por eso el DPA con cada empresa cliente (`21719-dpa.md`) debe autorizar expresamente a
> Supabase, Google y Zoom como sub-encargados. Sin esa autorización, la transferencia carece de amparo.
...
3. **Incluir la lista de sub-encargados en el DPA con cada empresa cliente**, para que la subcontratación
   quede autorizada.
4. **Archivar la evidencia** de cada aceptación en esta carpeta. Ante una fiscalización, lo que se pide
