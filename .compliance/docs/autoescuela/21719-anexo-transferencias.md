...
- **Exportador de datos:** Jorge Enrique Pérez Godoy Capacitación y Servicios EIRL, RUT 76.007.217-6,
  domicilio en Maipón 418, comuna de Chillán, Provincia de Diguillín, Región de Ñuble (Chile).
...
|---|---|---|---|---|---|---|---|
| 1 | **Supabase Inc.** | Estados Unidos `[CONFIRMAR la región concreta del proyecto en el panel de Supabase — puede ser EE.UU. o Sudamérica; si el proyecto está en São Paulo, el destino es Brasil]` | La totalidad de la base de datos: identificación de alumnos, relatores y trabajadores, expedientes con **certificado médico (si el alumno lo sube) y cédula**, académicos, datos laborales de alumnos enviados por empresas clientes, pagos, registros de auditoría, direcciones IP de consentimiento | Infraestructura de base de datos, almacenamiento de archivos y autenticación | Encargado | Cláusulas modelo | ❌ **Pendiente de incorporar** |
| 2 | **Google LLC (Gmail)** | Estados Unidos | Todo lo que un titular envíe al canal de datos para ejercer sus derechos, **incluida la imagen de su cédula de identidad**, más la correspondencia general | Alojamiento de la casilla `otecchillan@gmail.com`, que es el canal de ejercicio de derechos publicado en la política | Encargado | Cláusulas modelo | ❌ **Pendiente de incorporar** |
| 2-bis | **Proveedor de hosting del dominio `autoescuelachillan.cl`** `[COMPLETAR: razón social con la que factura a la Autoescuela]`. Opera el servidor `rs7-va.serverhostgroup.com` sobre infraestructura de **OVH SAS** | **Estados Unidos** — Warrenton, Virginia (verificado 17-08-2026 por la IP pública del servidor, `40.160.21.52`) | Nombre y correo de alumnos, relatores e instructores, y el contenido de los correos salientes, incluidos los certificados adjuntos | Alojamiento del sitio público y **servidor de correo saliente** (SMTP autenticado sobre TLS) | Encargado | Cláusulas modelo | ❌ **Pendiente de incorporar** — y requiere además contrato de tratamiento (Anexo 8, Modelo A) |
...
> **Nota propia de esta sociedad:** parte de los datos que viajan al extranjero **no son propios, sino de
> las empresas clientes**, en el escenario contingente de la sección 5 del Anexo 1 —que hoy no ocurre—
> en que una empresa encargue capacitar a una nómina de trabajadores. Respecto de esos datos actuaría
> como **encargado**, y **subcontratar infraestructura extranjera requiere la autorización de la empresa
> cliente**. Por eso el contrato con cada empresa cliente (**Anexo 8, Modelo B, cláusula B.4**) debe
> autorizar expresamente a Supabase, Google y Zoom como sub-encargados. Sin esa autorización, la
> transferencia carece de amparo.
...
1. **Confirmar la región** en que está alojada la infraestructura contratada, en el panel de
   ...
   sudamericana, corregir la fila 1 **y también** los Anexos 1, 2, 3 y 8, que hoy declaran Estados
   Unidos.
...
3. **Incluir la lista de sub-encargados en el contrato con cada empresa cliente** (Anexo 8, Modelo B,
   cláusula B.4), para que la subcontratación quede autorizada.
4. **Archivar la evidencia** de cada aceptación junto a este expediente. Ante una fiscalización, lo que
   se pide es el documento, no la afirmación de que existe.
5. Repetir el paso al **contratar cualquier proveedor nuevo** y al renovar los existentes.
