# Asignación ASG-i-004 — Migrar funciones de negocio del cliente a Edge Functions
...
> ⚠️ **Renumerada de `ASG-i-002` a `ASG-i-004` al mergear (2026-08-24)** — el ID `ASG-i-002` ya
> estaba tomado en `main` por una asignación distinta con el mismo título literal
> ("Migrar funciones de negocio del cliente a Edge Functions"), ya **completada**
> (`0011-m-print-flows-edge-functions`). Esa asignación acotó el alcance a 3 flujos de
> impresión concretos (`ficha-tecnica-print`, `route-sheet-print`, `epq-print`) que generaban
> HTML client-side — leer su nota de "Corrección de alcance" antes de reclamar esta, para no
> reauditar terreno ya cubierto. Esta asignación sigue siendo válida como auditoría más amplia
> (Facades/services en general, no solo los 3 flujos de impresión ya migrados).
...
- Excluir del alcance los 3 flujos de impresión ya resueltos en `ASG-i-002` /
  `0011-m-print-flows-edge-functions`.

## Referencias

- `ASG-i-002-funciones-de-negocio-a-edge-functions.md` (predecesora, completada — acotó y
  resolvió los 3 flujos de impresión client-side)
