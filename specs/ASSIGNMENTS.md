...
| ID | Título | Asignado a | Tipo sugerido | Prioridad | Creado por | Notas |
|----|--------|-----------|---------------|-----------|------------|-------|
| ASG-b-036 | 🔴 Ciclo de vida de la clase: exclusión mutua, cierre automático y aviso | `i` | spec | **Alta** | b | **BLOQUEADA.** Hallazgo verificado: `startClass()` no valida nada y una clase `in_progress` **nunca se cierra sola** (el cron solo toca `scheduled`). Agrupa 4 anotaciones. ⚠️ Solapa con ASG-b-010. **2026-08-01: nota agregada, no cambia el alcance** — ya existe un aviso a secretaría por cierre **manual** (fix-091-m/fix-092-m, vía ASG-b-044) reutilizable como patrón; el alcance completo de esta asignación (exclusión mutua + cierre automático + su aviso) sigue pendiente, ver notas del archivo de la asignación |
