...
- [x] Instructor: iniciar clase (KM inicial) → estado pasa a "en curso" — verificado 2026-08-13. Flujo rediseñado en [fix-175-m-instructor-clase-cierre-simplificado](../specs/fixes/fix-175-m-instructor-clase-cierre-simplificado/fix.md) (tras iniciar, vuelve al dashboard en vez de saltar directo al cierre) + [fix-176-m-dashboard-instructor-clases-activas-timezone](../specs/fixes/fix-176-m-dashboard-instructor-clases-activas-timezone/fix.md) (bug de timezone que hacía desaparecer la clase de "Mis Clases de Hoy" pasadas las ~21:00, más indicador visual claro "Clase en curso — recuerda finalizarla al terminar"). Confirmado en pantalla: badge "En Curso" + botón pulsante + texto explícito.
- [ ] Instructor: intentar iniciar una segunda clase mientras otra está "en curso" → bloqueado (exclusión mutua)
- [ ] Instructor: finalizar clase (KM final, observaciones y firmas ambos opcionales) → estado pasa a "completada", asistencia registrada
...
- [ ] Registrar un pago (abono) a una matrícula → estado de deuda del alumno se actualiza
- [ ] Aplicar un descuento predefinido → monto se recalcula correctamente (% y fijo)
- [ ] Intentar duplicar un pago ya registrado → comportamiento idempotente, no se duplica
- [ ] Registrar un egreso / gasto fijo → aparece en reportes de cuadratura
...
- [ ] Ver "Clases en vivo" en dashboard → refleja clases realmente `in_progress` en tiempo real (abrir en 2 pestañas y verificar Realtime)Ñ
